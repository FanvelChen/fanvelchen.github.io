...
Then choose $f^{(1)}$ , it shouldn't be linear. We choose **activation function** after affine transformation $\boldsymbol{h} = g(\boldsymbol{W}^T\boldsymbol{x}+\boldsymbol{c})$ , and respectively $\boldsymbol{h}_i$ $ = $ $g(\boldsymbol{x}^T \boldsymbol{W}_{:,i}+\boldsymbol{c}_i)$
